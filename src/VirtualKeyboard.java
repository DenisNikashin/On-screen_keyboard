...
public class VirtualKeyboard {
  ...
  public VirtualKeyboard(ReadOnlyObjectProperty<Node> target) {
    ...
    final String[][] unshiftedRU = new String[][]{
            {"ё", "1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "-", "="},
            {"й", "ц", "у", "к", "е", "н", "г", "ш", "щ", "з", "х", "ъ", "\\"},
            {"ф", "ы", "в", "а", "п", "р", "о", "л", "д", "ж", "э"},
            {"я", "ч", "с", "м", "и", "т", "ь", "б", "ю", "."}};

    final String[][] shiftedRU = new String[][]{
            {"Ё", "1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "-", "="},
            {"Й", "Ц", "У", "К", "Е", "Н", "Г", "Ш", "Щ", "З", "Х", "Ъ", "\\"},
            {"Ф", "Ы", "В", "А", "П", "Р", "О", "Л", "Д", "Ж", "Э"},
            {"Я", "Ч", "С", "М", "И", "Т", "Ь", "Б", "Ю", ","}};
    ...
    createRowKeyboard(target, modifiers, unshifted, shifted,
            unshiftedRU, shiftedRU, codes, extraLeftButtons, extraRightButtons);
    ...
    bottomRow.getChildren().addAll(modifiers.ctrlKey(), modifiers.altKey(),
            modifiers.metaENGKey(), spaceBar, cursorLeft, cursorUpDown, cursorRight);
    root.getChildren().add(bottomRow);
  }

  /**
   *  creates rows for both languages on the keyboard
   * */
  private void createRowKeyboard(ReadOnlyObjectProperty<Node> target,
                                 Modifiers modifiers,
                                 String[][] unshifted,
                                 String[][] shifted,
                                 String[][] unshiftedRU,
                                 String[][] shiftedRU,
                                 KeyCode[][] codes, Node[][] extraLeftButtons, Node[][] extraRightButtons) {
    for (int row = 0; row < unshifted.length; row++) {
      ...
      createColumnKeyboard(target, modifiers, unshifted[row], shifted[row],
              unshiftedRU[row], shiftedRU[row], codes[row], hbox);
      hbox.getChildren().addAll(extraRightButtons[row]);
    }
  }

  /**
   *  creates columns for both languages on the keyboard
   *  @param strings accepts unshifted String array
   *  @param strings1 accepts shifted String array
   *  @param strings2 accepts unshiftedRU String array
   *  @param strings3 accepts shiftedRU String array
   *
   * */
  private void createColumnKeyboard(ReadOnlyObjectProperty<Node> target, Modifiers modifiers,
                                    String[] strings, String[] strings1, String[] strings2,
                                    String[] strings3, KeyCode[] code, HBox hbox) {
    for (int k = 0; k < strings.length; k++) {
      hbox.getChildren().add(createShiftableButton(strings[k], strings1[k], strings2[k],
              strings3[k], code[k], modifiers, target));
    }
  }
  ...
  /**
   * this method for switching languages on the keyboard
   * The "text" variable receives the result by pressing ToggleButton 'ENG'
   * */
  private Button createShiftableButton( String unshifted,  String shifted,
                                        String unshiftedRU, String shiftedRU,
                                        final KeyCode code, Modifiers modifiers,
                                        final ReadOnlyObjectProperty<Node> target) {

    final ReadOnlyBooleanProperty letter = new SimpleBooleanProperty(unshifted.length() == 1
            && Character.isLetter(unshifted.charAt(0)));

    ObservableStringValue text = Bindings.when(modifiers.metaENGDown())
            .then(Bindings.when(modifiers.shiftDown().or(modifiers.capsLockOn().and(letter)))
                    .then(shiftedRU)
                    .otherwise(unshiftedRU))

            .otherwise(Bindings.when(modifiers.shiftDown().or(modifiers.capsLockOn().and(letter)))
                    .then(shifted)
                    .otherwise(unshifted));
  ...
  private KeyEvent createKeyEvent(Object source, EventTarget target, EventType<KeyEvent> eventType,
    ...
    return new KeyEvent(source, target, eventType, character, code.toString(),
            ...
            modifiers.altDown().get(), modifiers.metaENGDown().get());
  }
  ...
  private static class Modifiers {
    ...
    private final ToggleButton metaENG;
    ...
    public BooleanProperty metaENGDown() {
      return metaENG.selectedProperty();
    }
 ...
 *//*
          //      hbox.getChildren().add(createShiftableButton(unshiftedRU[row][k], shiftedRU[row][k], codes[row][k], modifiers, target));

 handleEnAlphabet(target, modifiers, unshifted[row], shifted[row], codes[row], hbox);

      handleRuAlphabet(target, modifiers, unshiftedRU[row], shiftedRU[row], codes[row], hbox);

 private void handleEnAlphabet(ReadOnlyObjectProperty<Node> target, Modifiers modifiers, String[] strings, String[] strings1, KeyCode[] code, HBox hbox) {

      for (int k = 0; k < strings.length; k++) {
        hbox.getChildren().add(createShiftableButton(strings[k], strings1[k], code[k], modifiers, target));
      }

  }

  private void handleRuAlphabet(ReadOnlyObjectProperty<Node> target, Modifiers modifiers, String[] strings, String[] strings1, KeyCode[] code, HBox hbox) {
    if (modifiers.meta == null) {
      for (int k = 0; k < strings.length; k++) {
        hbox.getChildren().add(createShiftableButton(strings[k], strings1[k], code[k], modifiers, target));
      }
    }
